...
/**************************************************************************************
 * @name vj
 * @arg [0] recovery, normal, off, on
 * @brief bench specific. 
 **************************************************************************************/
#define VJ_ARG_ACTION 0

#define VJ_DIO_POWER 4
#define VJ_DIO_JUMPER 6

#define VJ_ACTION_NONE 0
#define VJ_ACTION_RECOVERY 1
#define VJ_ACTION_NORMAL 2
#define VJ_ACTION_OFF 3
#define VJ_ACTION_ON 4

#define VJ_PS_DIO 4
#define VJ_JUMPER_DIO 6

#define VJ_IMMEDIATELY 0
#define VJ_DELAYNEXTACTION 150 // more or less 3 seconds
...
eResult cmd_vj(void)
{
    static eVj_state vjState = vj_getArg;
    static dioMsgType dioMsg;
    static uint8_t vjAction = VJ_ACTION_NONE;
    ...
    /****** *********************************** *****/
    case vj_getArg:
        if( eResult_OK == commandProcessor_compArg_s(VJ_ARG_ACTION, "recovery") ){
            myPgmspace_printSync(myProgmem_cmdResp_vjPending_id);
            vjState = vj_executeCmd;
            vjAction = VJ_ACTION_RECOVERY;
            result = eResult_PENDING;
        }
        else if( eResult_OK == commandProcessor_compArg_s(VJ_ARG_ACTION, "normal") ){
            myPgmspace_printSync(myProgmem_cmdResp_vjPending_id);
            vjState = vj_executeCmd;
            vjAction = VJ_ACTION_NORMAL;
            result = eResult_PENDING;
        }
        else if( eResult_OK == commandProcessor_compArg_s(VJ_ARG_ACTION, "off") ){
            myPgmspace_printSync(myProgmem_cmdResp_vjPending_id);
            vjState = vj_executeCmd;
            vjAction = VJ_ACTION_OFF;
            result = eResult_PENDING;
        }
        else if( eResult_OK == commandProcessor_compArg_s(VJ_ARG_ACTION, "on") ){
            myPgmspace_printSync(myProgmem_cmdResp_vjPending_id);
            vjState = vj_executeCmd;
            vjAction = VJ_ACTION_ON;
            result = eResult_PENDING;
        }
        else{
            myPgmspace_printSync(myProgmem_cmdResp_vjError_id);
            result = eResult_NOT_OK;
        }
    ...
    case vj_executeCmd:
        ...
        switch(vjAction){
        case VJ_ACTION_RECOVERY: /* -------------------------- */
            result = eResult_PENDING;
            if( delayCnt == VJ_IMMEDIATELY ){
                // power off
                dioMsg.action = DIO_ACTION_WRITE;
                dioMsg.gpioId = VJ_PS_DIO;
                dioMsg.level = DIO_LEVEL_HIGH;
                channelInterface_receiveDioMessage(&dioMsg);
                // jumper on
                dioMsg.action = DIO_ACTION_WRITE;
                dioMsg.gpioId = VJ_JUMPER_DIO;
                dioMsg.level = DIO_LEVEL_HIGH;
                channelInterface_receiveDioMessage(&dioMsg);
                // wait
            }
            else if( delayCnt == VJ_DELAYNEXTACTION ){ 
                // power on
                dioMsg.action = DIO_ACTION_WRITE;
                dioMsg.gpioId = VJ_PS_DIO;
    ...
    if( result != eResult_PENDING ){
        myPgmspace_printSync(myProgmem_cmdResp_vjDone_id);
        vjState = vj_getArg;
        vjAction = VJ_ACTION_NONE;
        delayCnt = 0;
    }
